...
и станет передовать данные получателю как только они станут доступны.
...
посылает `'data'` событие и имеет опциональнй метод [`pause()`][], тогда
...
Примеры writable stream включают:
...
Stream в **object mode** позволяют посылать генерированные JavaScript значения
