...
Streams могут быть либо [Readable][], либо [Writable][],
...
var server = http.createServer(function (req, res) {
  ...
  // При получении данных, Readable streams будет посылать событие 'data'.
...
```
Каждый раз, при чтении из буфера, данные станут доступны,
будет снова послано событие `readable`.
...
вы хотите полностью закончить приём и послать событие `end`,
...
stream-источник посылает `end` событие и
`получатель` становится больше не доступным для записи. Передача `{ end: false }`
как `параметр` в этот метод, сохраняет stream-получателя записываемым.

Так что мы сможем закрыть его в конце работы с `reader`:
...
Метод удаляет хуки установленные для предыдущего вызова `pipe()`.

Если получатель не указан, будут удалены все ранее указанные.

Если получатель указан, но передача данных в процессе - метод ничего не сделает.
...
* `chunk` {Buffer | String} Порция данных для извлечения из очереди чтения и сдвига его назад.
...
Когда вызван [`end()`][], и данные были отосланы в низкоуровневую систему,
...
* `options` {Object}
  * `highWaterMark` {Number}  Максимальное количество байтов
    ...
    из низкоуровневого ресурса. Значение по умолчанию = 16kb, или 16 для  `objectMode` streams.
...
* `options` {Object}
  * `highWaterMark` {Number} Уровень буфера, когда [`write()`][],
    возвращает false. Значение по умолчанию = 16kb, или 16 для  `objectMode` streams.
...
будут просто отбрасываться. Однако, в версияхio.js v1.0 и Node.js v0.10 и
